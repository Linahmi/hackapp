...
export function buildEscalations(triggers, estimatedSavings = null) {
  const rules = {
    "ER-001": { target: "Requester" },
    "ER-002": { target: "Procurement Manager" },
    "ER-003": { target: "Head of Strategic Sourcing" },
    "ER-004": { target: "Head of Category" },
    "ER-005": { target: "Security / Compliance" },
    "ER-006": { target: "Sourcing Excellence Lead" },
    "ER-007": { target: "Marketing Governance Lead" },
    "ER-008": { target: "Regional Compliance Lead" }
  };
  ...
  return triggers.map((t, i) => ({
    ...
    rule: t.rule,
    trigger: t.reason,
    escalate_to: rules[t.rule]?.target || 'Procurement Manager',
    action: rules[t.rule]?.action || '',
