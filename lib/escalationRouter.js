...
export function routeEscalations(validation, policyEvaluation, shortlist, request) {
  ...
  const invalidIssues = validation.issues_detected.filter(i =>
  ...
  const budgetInsufficient = validation.issues_detected.find(i => i.type === "budget_insufficient");
  ...
  const leadInfeasible = validation.issues_detected.find(i => i.type === "lead_time_infeasible");
  if (leadInfeasible) {
  ...
  const leadCritical = validation.issues_detected.find(i => i.type === "lead_time_critical");
  ...
  const restrictedCategory = validation.issues_detected.find(i => i.type === "restricted_category");
