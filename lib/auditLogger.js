...
export function logAuditEvent({ action, requestId, userId = null , metadata = {} }) {
  ...
  // Persist to DB first, fall back to file if DB unavailable.
  // Non-blocking: fire-and-forget so the pipeline is never held up.
  (async () => {
    try {
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || `http://localhost:${process.env.PORT || 3000}`;
      const response = await fetch(`${baseUrl}/api/db/audit`, {
        method: 'POST',
      ...
      if (!data.success) {
        // DB returned failure — fall back to file
      ...
      // fetch failed entirely — fall back to file
      console.warn('DB audit write failed, using file fallback:', e?.message);
      const existing = safeRead();
      existing.push(event);
      if (existing.length > 2000) existing.splice(0, existing.length - 2000);
      safeWrite(existing);
