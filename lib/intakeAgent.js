import { logLLMCall } from './transparencyLogger.js';
    ...
    const response = await fetch(
      azureChatUrl,
      {
        ...
        body: JSON.stringify({
          ...
          stream: !!onChunk,
          messages: [
            { role: 'system', content: 'You are a procurement intake agent. Extract structured data from purchase requests. Break down complex requests into atomic line items if there are multiple products requested.' },
            { role: 'user', content: `Parse this procurement request into a JSON object. If there are multiple distinct items (e.g. "50 laptops and 20 chairs"), extract them into an 'items' array.

Each item (in the 'items' array or as the main object) MUST have these fields:
- line_id: "line_1", "line_2", etc.
- category_l2: specific subcategory like Laptops, Docking Stations, Consulting, etc.
- quantity: integer number of units
- budget_amount: total budget for this line as a number
- currency: EUR, CHF, or USD
- delivery_countries: array of ISO 2-letter codes
- required_by_date: ISO date string or null
- preferred_supplier_stated: supplier name or null

Also include these global fields:
- detected_language: en, fr, de, es
- unclear_intent: boolean
- items: array of objects (if multiple items), otherwise null
...
Return ONLY the JSON object.` }
    ...
    if (!onChunk) {
      ...
      logLLMCall('IntakeAgent', {
        endpoint: azureChatUrl,
