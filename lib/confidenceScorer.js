...
export function explainConfidence(request, validation, policyEvaluation, shortlist, escalations = [], recommendation = null) {
  ...
  const secondSupplier = shortlist?.[1] ?? null;

  if (topSupplier && secondSupplier) {
    const gap = Math.abs((topSupplier.composite_score || 0) - (secondSupplier.composite_score || 0));
    if (gap < 3) {
      score -= 12;
      pushDriver(drivers, "warn", "Narrow margin between top suppliers creates uncertainty");
    }
  }
