...
import { computeFuzzyScore } from "./fuzzyScorer.js";
...
export function scoreSuppliers(eligibleSuppliers, policyEvaluation, request) {
  const { quantity, days_until_required, currency = "EUR", historicalContext = [] } = request;
  ...
  const scored = candidates.map((c) => {
    ...
    // --- Fuzzy Suitability Overlay (Mamdani Inference) ---
    const fuzzySuitability = computeFuzzyScore(priceScore * 100, qualityScore * 100);
    const combinedScore = (baseScore * 0.8) + (fuzzySuitability / 100 * 0.2);

    // --- Historical Fixed-Point Adjustment (absolute points, NOT percentages) ---
    let historicalPts = 0;
    let historicalFlags = [];
    const supplierRecords = historicalContext.filter(h => h.supplier_id === c.supplier.supplier_id);
    const awards = supplierRecords.filter(r => r.awarded);
    const rejections = supplierRecords.filter(r => !r.awarded);
    const now = new Date();
    const twelveMonthsAgo = new Date(now.getFullYear() - 1, now.getMonth(), now.getDate());

    // +3 pts (1–2 awards) or +5 pts (3+ awards)
    if (awards.length >= 3) {
      historicalPts += 5;
      historicalFlags.push(`+5 pts based on ${awards.length} past awards`);
    } else if (awards.length >= 1) {
      historicalPts += 3;
      historicalFlags.push(`+3 pts based on ${awards.length} past award(s)`);
    }

    // +2 pts (recent) — at least one award in last 12 months
    const hasRecent = awards.some(a => a.award_date && new Date(a.award_date) >= twelveMonthsAgo);
    if (hasRecent) {
      historicalPts += 2;
      historicalFlags.push("+2 pts recent award (last 12 months)");
    }

    // +2 pts (reliable) — has awards AND none had escalations
    if (awards.length > 0 && !awards.some(a => a.escalation_required)) {
      historicalPts += 2;
      historicalFlags.push("+2 pts reliable (no past escalations)");
    }

    // -3 pts (1 rejection) or -6 pts (2+ rejections)
    if (rejections.length >= 2) {
      historicalPts -= 6;
      historicalFlags.push(`-6 pts based on ${rejections.length} past rejections`);
    } else if (rejections.length === 1) {
      historicalPts -= 3;
      historicalFlags.push("-3 pts based on 1 past rejection");
    }

    // -1 pt (inactive) — has records but NO award in last 12 months
    if (supplierRecords.length > 0 && !hasRecent) {
      historicalPts -= 1;
      historicalFlags.push("-1 pt inactive (no award in last 12 months)");
    }

    // Hard clamp: total adjustment must be between -8 and +8
    const clampedPts = Math.max(-8, Math.min(8, historicalPts));
    if (clampedPts !== historicalPts) {
      historicalFlags.push(`Clamped from ${historicalPts} to ${clampedPts} pts`);
    }

    // Each historical point = 1% shift in composite score
    const HISTORICAL_WEIGHT = 0.01;
    const finalScore = Math.max(0, Math.min(1, combinedScore + clampedPts * HISTORICAL_WEIGHT));

    return {
      ...c,
      scoreBreakdown: { price: priceScore, lead_time: leadScore, quality: qualityScore, risk: riskScore, esg: esgScore, historical: clampedPts, fuzzy_suitability: fuzzySuitability / 100 },
      historicalFlags,
  ...
  const shortlist = scored.map((c, idx) => {
    ...
    return {
      ...
      historical_flags: c.historicalFlags,
