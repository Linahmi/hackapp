...
export function scoreSuppliers(eligibleSuppliers, policyEvaluation, request) {
  ...
  const scored = candidates.map((c) => {
    ...
    const finalScore = Math.max(0, Math.min(1, baseScore + clampedPts * HISTORICAL_WEIGHT));

    return {
      ...c,
      scoreBreakdown: { price: priceScore, lead_time: leadScore, quality: qualityScore, risk: riskScore, esg: esgScore, historical: clampedPts },
      historicalFlags,
