...
function buildRationale(hasBlocking, rankedSuppliers, validationResult, escalations, structuredRequest) {
  if (!hasBlocking) {
    ...
    return `Based on the analysis, ${top?.supplier_name || 'the top supplier'} is recommended with a composite score of ${top?.composite_score}. This supplier offers the best balance of price, lead time, quality and ESG compliance.`;
  }
