...
async function callAzure(systemPrompt, userPrompt) {
  ...
  try {
    ...
    console.error('[decisionEngine] Error calling Azure:', error);
    return null;
  }
}
...
CRITICAL: Also include a "Counterfactual Feedback" section for the runner-up ${runner ? runner.supplier_name : 'none'}. Explain exactly what they would need to improve (e.g., lower price by X, or faster lead time by Y days) to have taken the #1 rank.`;
