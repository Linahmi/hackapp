...
function buildMockDecision(hasBlocking, rankedSuppliers, validationResult, escalations, structuredRequest) {
  ...
  if (!hasBlocking && top) {
    return {
      ...
      justification: `Ranked first with a composite score of ${top.composite_score}/100, offering the strongest balance of price (${top.unit_price} ${structuredRequest.currency}/unit), lead time (${top.standard_lead_time_days} days), and compliance. Budget is sufficient and delivery is feasible within the required window.`,
      next_action: 'Client to validate the order — no further approvals required.',
      key_reasons: [
        `Best composite score: ${top.composite_score}`,
