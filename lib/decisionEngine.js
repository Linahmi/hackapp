export async function generateDecision(structuredRequest, validationResult, policyResult, rankedSuppliers, escalations, historicalContext, onChunk = null) {
  ...
  if (USE_MOCK) {
    ...
    const blockingTriggers = escalations.filter(e => e.blocking).map(e => e.trigger);
    const mockRationale = hasBlocking
      ? `This request cannot proceed automatically. Blocking issue(s): ${blockingTriggers.join(' | ')}. Human intervention is required to resolve these before sourcing can continue.`
