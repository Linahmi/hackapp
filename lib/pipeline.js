import { getData, getEligibleSuppliers, fuzzyMatchCategory, getGlobalMedianUnitPrice } from './dataLoader.js';
...
const HARD_BLOCK_CASE_TYPES = ['FAILED_IMPOSSIBLE_DATE', 'MORE_INFO_REQUIRED', 'NO_SUPPLIER_AVAILABLE', 'PENDING_RESOLUTION', 'OUT_OF_SCOPE'];
...
export async function runPipeline({ text, request_id, session } = {}, { onStep } = {}) {
  ...
  // ── CSV scoring — with fuzzy-category fallback ──────────────────────────
  let { shortlist: csvSuppliers, excluded: excludedSuppliers } = scoreForPipeline(
  ...
  let fuzzyMatchApplied = null;
  if (csvSuppliers.length === 0) {
    const categoryQuery = enrichedRequest.category_l2 ?? enrichedRequest.category_l1 ?? '';
    const fuzzy = fuzzyMatchCategory(categoryQuery);
    if (fuzzy) {
      const { shortlist: fuzzySuppliers, excluded: fuzzyExcluded } = scoreForPipeline(
        fuzzy.category_l1, fuzzy.category_l2,
        enrichedRequest.delivery_countries || [], safeQty, enrichedRequest.currency || 'EUR',
        originalRequest, enrichedRequest.days_until_required, enrichedRequest.budget_amount, historicalContext,
      );
      if (fuzzySuppliers.length > 0) {
        csvSuppliers = fuzzySuppliers;
        excludedSuppliers = fuzzyExcluded;
        fuzzyMatchApplied = fuzzy;
      }
    }
  }

  emit({ step: 'scoring', status: 'active', pct: 45, thinking: `Found ${csvSuppliers.length} eligible supplier(s) for ${enrichedRequest.category_l2 ?? enrichedRequest.category_l1 ?? 'this category'} in [${(enrichedRequest.delivery_countries ?? []).join(', ')}]${fuzzyMatchApplied ? ` (fuzzy-matched → ${fuzzyMatchApplied.category_l2})` : ''} · Scoring on price (30%), lead time (30%), quality (20%), risk (10%), ESG (10%)…` });
  ...
  // OUT_OF_SCOPE: category was parsed correctly but no CSV or Exa suppliers found
  const isOutOfScope = !hasUnclearIntent && rankedSuppliers.length === 0 && exaCandidates.length === 0 && csvSuppliers.length === 0;
  ...
  else if (isOutOfScope)                                                   case_type = 'OUT_OF_SCOPE';
  else if (rankedSuppliers.length === 0)                                   case_type = 'NO_SUPPLIER_AVAILABLE';
  else if (fuzzyMatchApplied || (structuredRequest.demand_reframe_flag && rankedSuppliers.length > 0)) case_type = 'SIMILAR_NOT_EXACT_MATCH';
  ...
  const result = {
    ...
    audit_trail: {
      ...
      fuzzy_category_match:         fuzzyMatchApplied ? { matched_to: `${fuzzyMatchApplied.category_l1} › ${fuzzyMatchApplied.category_l2}`, score: Math.round(fuzzyMatchApplied.matchScore * 100) } : null,
...
function buildExaShortlistEntries(exaDiscovery, csvSuppliers, quantity, currency, budgetAmount) {
  ...
  // Use median unit price from CSV shortlist — fall back to global pricing median when category had no CSV match
  const sortedPrices = csvSuppliers.map(s => s.unit_price).filter(Boolean).sort((a, b) => a - b);
  const medianUnitPrice = sortedPrices[Math.floor(sortedPrices.length / 2)] ?? getGlobalMedianUnitPrice();
  if (!medianUnitPrice) return [];
  ...
  for (let i = 0; i < liveCandidates.length; i++) {
    ...
    entries.push({
      ...
      recommendation_note: csvSuppliers.length === 0
        ? 'External candidate from live market search. Category is outside the approved panel — pricing is a rough market estimate. Full RFQ required before award.'
        : 'External candidate from live market search. Pricing is estimated from comparable approved suppliers — obtain RFQ before award.',
      tco: totalPrice,
