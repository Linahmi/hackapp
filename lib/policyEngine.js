...
export function checkApprovalTier(totalValue, currency) {
  ...
  for (let i = 0; i < thresholds.length; i++) {
    ...
    if (totalValue >= min && totalValue <= max) {
      let approver = null;
      let approvers = [];
      let deviationApproval = null;
      if (t.deviation_approval_required_from?.length > 0) {
        approver = t.deviation_approval_required_from[0];
        approvers = t.deviation_approval_required_from;
        deviationApproval = t.deviation_approval_required_from.join(", ");
        ...
        approvers = t.approvers.map((role) => maps[role] || role);
        approver = approvers[approvers.length - 1];
      } else if (t.managed_by?.length > 0) {
        approvers = t.managed_by.map((role) => role === "business" ? "Business" : "Procurement Manager");
        approver = approvers[approvers.length - 1];
      }

      return {
        tier: i + 1,
        rule_applied: t.threshold_id ?? `AT-00${i + 1}`,
...
export function checkPreferredSupplier(supplierName, categoryL2, deliveryCountry) {
  const { suppliers, policies } = getData();
  const supplier = suppliers.find(s =>
    s.supplier_name === supplierName &&
    s.category_l2 === categoryL2
  );

  // Check policies.restricted_suppliers by name only — catches cross-category restrictions
  const policyRestriction = policies.restricted_suppliers?.find(
    r => r.supplier_name?.toLowerCase() === supplierName.toLowerCase()
  );

  if (!supplier && !policyRestriction) return null;

  const regions = supplier?.service_regions
    ? (Array.isArray(supplier.service_regions) ? supplier.service_regions : supplier.service_regions.split(';'))
    : [];
  const coversCountry = supplier ? regions.includes(deliveryCountry) : false;
  const isPreferred = supplier
    ? (supplier.preferred_supplier === "True" || supplier.preferred_supplier === true || supplier.preferred_supplier === "true")
    : false;
  const isRestricted =
    (supplier?.is_restricted === "True" || supplier?.is_restricted === true || supplier?.is_restricted === "true") ||
    !!policyRestriction;
  ...
  return {
    ...
    restriction_reason: policyRestriction?.restriction_reason || null
  };
}
